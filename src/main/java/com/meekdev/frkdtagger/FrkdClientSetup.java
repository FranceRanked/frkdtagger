...
import com.meekdev.frkdtagger.config.gui.ConfigScreen;
import com.meekdev.frkdtagger.input.Keybinds;
import com.meekdev.frkdtagger.render.CardPrewarm;
import com.meekdev.frkdtagger.render.CardScreen;
import com.meekdev.frkdtagger.render.InfoPanel;
import com.meekdev.frkdtagger.render.ProfilePopupRenderer;
import com.meekdev.frkdtagger.update.FrkdUpdater;
...
final class FrkdClientSetup {
    private FrkdClientSetup() {
    }

    //? if >=26 {
    /*private static final KeyMapping.Category CATEGORY =
            KeyMapping.Category.register(Identifier.fromNamespaceAndPath(FrkdTagger.MOD_ID, "frkd"));
    *///?}
    ...
    private static void createKeys() {
        Keybinds.popup = key("key.frkdtagger.popup");
        Keybinds.config = key("key.frkdtagger.config");
        Keybinds.cycle = key("key.frkdtagger.cycle");
    }
    ...
    static void handleTick() {
        ...
        if (Keybinds.cycle != null) {
            while (Keybinds.cycle.consumeClick()) {
                String mode = FrkdTagger.cycleGameMode();
                if (mc.player != null) {
        ...
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, access) ->
                dispatcher.register(lit("frkd")
                        ...
                        .then(lit("toggle").executes(ctx -> {
                            boolean on = !FrkdTagger.config().enabled();
                            FrkdTagger.config().setEnabled(on);
                        ...
                        .then(lit("kit")
                                .then(arg("kit", StringArgumentType.word()).executes(ctx -> {
                                    ...
                                    FrkdTagger.config().setGameMode(kit);
