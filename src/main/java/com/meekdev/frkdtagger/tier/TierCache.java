...
import com.meekdev.frkdtagger.api.Mode;
...
public final class TierCache {
    ...
    public static void refreshModes() {
        FranceRankedApi.modes()
                .thenAccept(modes -> {
                    ...
                    FrkdTagger.LOGGER.info("Loaded {} FranceRanked modes", MODES.size());
                })
                .exceptionally(t -> {
                    FrkdTagger.LOGGER.warn("Failed to load FranceRanked modes", t);
                    return null;
                });
    }
    ...
    private static boolean isStale(Entry entry) {
        long ttl = FrkdTagger.config().cacheTtlSeconds() * 1000L;
